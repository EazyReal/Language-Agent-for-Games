...
class MinimaxAgent:
    ...
    def minimax(self, board, player_index):
        ...
        action_space = self.current_observation['action_mask']
        board = self.current_observation['observation']
        
        if self.has_player_won(board, player_index) or self.is_draw(action_space):
            return self.game_state(board, player_index), None
        
        if player_index == 0: # the second player [::0]
        ...
        for action in action_space:
            ...
            sim_score = self.minimax(board, 1 - player_index)  # simulate a game after making that move

            # undo move
            self.do_move(board, action, player_index, 0)
            sim_score['position'] = action  # this represents the move optimal next move

            if player_index == 0:  # X is max player
    ...
    def act(self):
        ...
        best_dict = self.minimax(board, int(p2>p1))
        action = best_dict['position']
        v = best_dict['score']
        # action = self.minimax(board, int(p2>p1))['position']
        # v = self.minimax(board, int(p2>p1))['score']
